...
当转账超过所有者设定的守护人审批阈值时，守护人将可以直接从待审批请求发起应用内语音或视频通话。收款人、金额和交易指纹会与通话显示在同一上下文中，方便家人在不切换到 Zalo、另行拨打电话或使用其他聊天应用的情况下核对所有者的真实意图。

通话只是额外的协调层，**不是授权因子**。它不能自行批准、签名或广播转账：确定性的支出策略、守护人批准和钱包所有者的加密签名仍然是必需步骤。该功能**属于路线图，尚未实现**。发布前必须对呼叫信令、加密、元数据保留和滥用防护进行威胁建模。所有受信任设备均丢失后的钱包恢复，仍然是一个独立的身份核验问题。
